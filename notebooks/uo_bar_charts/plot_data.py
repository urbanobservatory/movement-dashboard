...
def plot_data_group(bar_chart,group,y_axis,last_monday=None):

    for group_name,sensor_names in group.items():
        group_data_frames = []
        for db_name in sensor_names:

            if not os.path.exists('{data_folder}/{sensor_name}.tar.gz'.format(
                sensor_name=db_name,
            ...
            if df.empty:
                continue
            re = df
            re.index = pd.to_datetime(re['Timestamp'])

            days = list(re.resample('86400S').sum()['Value'])

            if 0 in days or len(days) < 21:
                continue
            group_data_frames.append(df)
        if len(group_data_frames) == 0:
            continue
        df = pd.concat(group_data_frames)
        if df.empty:
            continue
        ...
        if pd.isnull(max_monday):
            continue
        last_monday = datetime.datetime(int(max_monday.year), int(max_monday.month), int(max_monday.day))
        ...
        ax1.set_ylabel(y_axis, fontweight='bold')
        ...
        fig_path = '{data_folder}/{group}.png'.format(
            group=group_name,
            data_folder=bar_chart.plots_folder
        )
        plt.savefig(fig_path)

        yield group_name,fig_path
        plt.close()


def plot_data(bar_chart,rename_function,y_axis):
